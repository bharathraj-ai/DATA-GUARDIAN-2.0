...
export default function OwnerDashboardClient({ initialLinks, initialUserId }: Props) {
    ...
    // Soft background refresh (UI already has server-prefetched links)
    useEffect(() => {
        if (sessionStatus !== 'authenticated' || !userId) return;
        let cancelled = false;
        const refresh = async () => {
            try {
                const owned = await getOwnedLinks(userId);
                if (!cancelled) setLinks(owned);
            } catch (error) {
                console.error('Error refreshing links:', error);
            }
        };
        // Skip immediate refetch — data came from the server page. Poll quietly.
    ...
    useEffect(() => {
        if (activeTab !== 'history' || historyLoaded || !userId) return;
        let cancelled = false;
        (async () => {
    ...
    useEffect(() => {
        if (activeTab !== 'audit' || auditLoaded || !userId) return;
        let cancelled = false;
        (async () => {
            setAuditLoading(true);
            try {
                const audits = await getUnifiedAuditLogs();
                if (!cancelled) {
                    setAuditLogs(audits);
                    setAuditLoaded(true);
                }
    ...
    return (
        <main className="app-page">
            <style>{`
                ...
                .detail-label {
                    color: #64748B; min-width: 120px; font-weight: 500;
                    ...
                    <div className="app-container" style={{ maxWidth: '960px' }}>
                                ...
                                {(['all', 'active', 'used', 'expired', 'revoked'] as const).map((f) => (
                                    <button
                                        ...
                                        style={{
                                            ...
                                            background: filter === f ? 'var(--primary-blue)' : '#F8FAFC',
                        ...
                        {activeTab === 'audit' && (
                            <div className="app-form-card">
                                {auditLoading || (!auditLoaded && activeTab === 'audit') ? (
                                    ...
                                    <div>
                                        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', marginBottom: '20px', background: '#F8FAFC', padding: '16px', borderRadius: '12px', border: '1px solid #E2E8F0' }}>
                                            <input
                                                type="text"
                                                placeholder="Search logs..."
                                                value={auditFilter.search}
                                                onChange={(e) => setAuditFilter({ ...auditFilter, search: e.target.value })}
                                                style={{ flex: 1, minWidth: '200px', padding: '8px 12px', borderRadius: '8px', background: '#F8FAFC', color: '#0F172A', border: '1px solid #E2E8F0' }}
                                            />
                                            <select
                                                value={auditFilter.type}
                                                onChange={(e) => setAuditFilter({ ...auditFilter, type: e.target.value })}
                                                style={{ padding: '8px 12px', borderRadius: '8px', background: '#F8FAFC', color: '#0F172A', border: '1px solid #E2E8F0' }}
                                            >
                                                ...
                                                {filteredAuditLogs.map(log => {
                                                    ...
                                                    return (
                                                        <div key={log.id} style={{
                                                            display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 0', borderBottom: '1px solid #E2E8F0'
                                                        }}>
                                                    ...
                                                    <div style={{
                                                        ...
                                                        <div style={{ flex: '0 0 auto' }}>
                                                            ...
                                                            <p style={{
                                                                ...
                                                                color: record.fileCount > 0 ? '#6d28d9' : 'var(--text-muted)',
                                                            }}>
                                        ...
                                        {filteredLinks.map((link) => (
                                            ...
                                            >
                                                ...
                                                <div style={{
                                                    ...
                                                    <button
                                                        ...
                                                        style={{
                                                            ...
                                                            background: copiedId === 'link-' + link.id ? 'rgba(34,197,94,0.15)' : '#F1F5F9',
                                                ...
                                                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
                                                    ...
                                                    >
                                                        {detailsLoadingId === link.id ? '⏳' : expandedId === link.id ? '▲ Less' : '▼ Details'}
                                                    </button>
                                                                ...
                                                                <div style={{
                                                                    ...
                                                                    {link.files.map((file, i) => (
                                                                        <div key={i} style={{
                                                                            ...
                                                                            borderBottom: i < link.files.length - 1 ? '1px solid #E2E8F0' : 'none',
                                                                        }}>
                                                                ...
                                                                <div style={{
                                                                    ...
                                                                    {link.auditLogs.map((log, i) => (
                                                                        <div key={i} style={{
                                                                            ...
                                                                            borderBottom: i < link.auditLogs.length - 1 ? '1px solid #E2E8F0' : 'none',
                                                                        }}>
